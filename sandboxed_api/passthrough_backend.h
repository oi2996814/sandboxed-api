...
class PassthroughBackend {
 public:
  ...
  PassthroughBackend(SandboxConfig config, CallFunctionT call_function)
      : rpc_channel_(std::move(call_function)) {}
  ...
  PassthroughRPCChannel* rpc_channel() { return &rpc_channel_; }

  absl::Status SetWallTimeLimit(absl::Duration limit) const {
    return absl::UnimplementedError("SetWallTimeLimit() not implemented");
  }
 ...
 private:
  PassthroughRPCChannel rpc_channel_;
};
